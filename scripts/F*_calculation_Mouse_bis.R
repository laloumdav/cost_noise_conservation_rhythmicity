...
## ## ## ## ## ## ## ## ## ## 
#*** LUNG ***#
load("Documents/rhythm_detection_benchmark/DATA/mouse_microarray/lung/facs_Lung_seurat_tiss.Robj")
cells.data <- tiss@meta.data
raw.data <- tiss@raw.data
cell.type.repl.nb <- plyr::count(cells.data$cell_ontology_class)
cell.type.repl.nb
# keep cells types with replicates number > 100
kept.repicates <- row.names(cells.data[cells.data$cell_ontology_class %in% cell.type.repl.nb[cell.type.repl.nb$freq>100, "x"], ])
raw.data <- raw.data[, kept.repicates]

#*** MUSCLE ***#
load("Documents/rhythm_detection_benchmark/DATA/mouse_microarray/muscle/facs_Limb_Muscle_seurat_tiss.Robj")
cells.data <- tiss@meta.data
raw.data <- tiss@raw.data
cell.type.repl.nb <- plyr::count(cells.data$cell_ontology_class)
cell.type.repl.nb
...
#*** HEART ***#
load("Documents/rhythm_detection_benchmark/DATA/mouse_microarray/heart/facs_Heart_seurat_tiss.Robj")
cells.data <- tiss@meta.data
raw.data <- tiss@raw.data
cell.type.repl.nb <- plyr::count(cells.data$cell_ontology_class)
cell.type.repl.nb
# keep cells types with replicates number > 100 and Leukocytes 
kept.repicates <- row.names(cells.data[cells.data$cell_ontology_class %in% cell.type.repl.nb[cell.type.repl.nb$freq>100, "x"][which((cell.type.repl.nb[cell.type.repl.nb$freq>100, "x"])!="leukocyte")], ])
raw.data <- raw.data[, kept.repicates]

#*** AORTA ***#
load("Documents/rhythm_detection_benchmark/DATA/mouse_microarray/aorta/facs_Aorta_seurat_tiss.Robj")
cells.data <- tiss@meta.data
raw.data <- tiss@raw.data
cell.type.repl.nb <- plyr::count(cells.data$cell_ontology_class)
cell.type.repl.nb
...
#*** KIDNEY ***#
load("Documents/rhythm_detection_benchmark/DATA/mouse_microarray/kidney/facs_Kidney_seurat_tiss.Robj")
cells.data <- tiss@meta.data
raw.data <- tiss@raw.data
cell.type.repl.nb <- plyr::count(cells.data$cell_ontology_class)
cell.type.repl.nb
...
data$Fstar6 <- exp(log(data$Variance) - predict(m6))
data$Fstar7 <- exp(log(data$Variance) - predict(m7))
data$Fstar8 <- exp(log(data$Variance) - predict(m8))

cor.test(~Fstar1+Mean, data, method = "kendall") 
cor.test(~Fstar2+Mean, data, method = "kendall") 
cor.test(~Fstar3+Mean, data, method = "kendall") 
cor.test(~Fstar4+Mean, data, method = "kendall") 
cor.test(~Fstar5+Mean, data, method = "kendall") 
cor.test(~Fstar6+Mean, data, method = "kendall") 
cor.test(~Fstar7+Mean, data, method = "kendall") 
cor.test(~Fstar8+Mean, data, method = "kendall") 
...
dat <- data[, c("Mean", "Variance", "sd", "noise adjusted sd", "Fano Factor", "Fstar1", "Fstar2", "Fstar3", "Fstar4", "Fstar5", "Fstar6", "Coeff Variation squared", "distance to median")]
#dat <- data.frame("ID"=rownames(dat), dat)
#write.table(dat, "~/Documents/rhythm_detection_benchmark/DATA/mouse_microarray/lung/noise_data.txt", sep = "\t", quote = F, row.names = F)
#write.table(dat, "~/Documents/rhythm_detection_benchmark/DATA/mouse_microarray/muscle/noise_data.txt", sep = "\t", quote = F, row.names = F)
#write.table(dat, "~/Documents/rhythm_detection_benchmark/DATA/mouse_microarray/heart/noise_data.txt", sep = "\t", quote = F, row.names = F)
#write.table(dat, "~/Documents/rhythm_detection_benchmark/DATA/mouse_microarray/aorta/noise_data.txt", sep = "\t", quote = F, row.names = F)
#write.table(dat, "~/Documents/rhythm_detection_benchmark/DATA/mouse_microarray/kidney/noise_data.txt", sep = "\t", quote = F, row.names = F)



dat <- data[, c("Mean", "Variance", "sd", "noise adjusted sd", "Fano Factor", "Fstar4", "Fstar5", "Coeff Variation squared", "distance to median")]
...
removeOutliersFunction <- function(x) {
  x[(x > quantile(x, 0.975)) | (x < quantile(x, 0.025))] <- NA
  return(x)
}

dat <- as.data.frame(apply(dat, 2, removeOutliersFunction))
#dat$`distance to median` <- dat$`distance to median` + abs(min(dat$`distance to median`, na.rm = TRUE))+.001
...
dat <- reshape::rename(dat, replace = c("Variance" = paste("Variance", "\n", "slope = ", slope.values[slope.values$SGEtype=="Variance", "slope.values"], sep=""), 
                                        ...
                                        "Fstar2" = paste("Fstar2", "\n", "slope = ", slope.values[slope.values$SGEtype=="Fstar2", "slope.values"], sep=""),
                                        ...
                                        "Fstar5" = paste("Fstar5", "\n", "slope = ", slope.values[slope.values$SGEtype=="Fstar5", "slope.values"], sep=""),
                                        "Fstar6" = paste("Fstar6", "\n", "slope = ", slope.values[slope.values$SGEtype=="Fstar6", "slope.values"], sep=""),
...
# Export as pdf image
#pdf("~/Documents/cost_theory_workingspace/supplementary_information/noiseDispersion_geneExpr_mouseKidney.pdf", height = 6, width = 4.5)
#print(p1)
#dev.off()
...
dat2 <- ddply(.data = dat, .variables = c("MeanClass", "SGEtype"),
              ...
              lo95 = quantile(SGEvalue, prob=0.025, na.rm=TRUE),
              hi95 = quantile(SGEvalue, prob=0.975, na.rm=TRUE))
...
# Export as pdf image
#pdf("~/Documents/cost_theory_workingspace/supplementary_information/noise_geomQuantiles_mouseKidney.pdf", height = 6, width = 4.5)
#print(p2)
#dev.off()
...
read_docx(path = "~/Documents/cost_theory_workingspace/figures_noise.docx") %>%
  ...
  cursor_reach(keyword = "##noise_estimations_variance_and_sd_MOUSE_Lung##") %>%
  body_add_gg(value = p2, width = 5, height = 6) %>%
  cursor_forward() %>%
  body_remove()  %>%
  
  cursor_reach(keyword = "##noise_histograms_variance_and_sd_MOUSE_Lung##") %>%
