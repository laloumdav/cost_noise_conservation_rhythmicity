...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  sub.dataframe.tissue <- subset(full.dataframe.tot, tissue == unique(full.dataframe.tot$tissue)[i])
  
  # Replace p-values = 0 by the min of p-values divided by 10
  min.value = min(sub.dataframe.tissue[sub.dataframe.tissue$GeneCycle.pvalue != 0, "GeneCycle.pvalue"])
  sub.dataframe.tissue[sub.dataframe.tissue$GeneCycle.pvalue == 0, "GeneCycle.pvalue"] <- min.value/10
  
  # See respective influence of gene expression and tissue-specificity into the rhythmic expression
  reg.lin <- lm(formula = -log10(GeneCycle.pvalue) ~ log(mean.RNA.level) + tau.mean, data = sub.dataframe.tissue)                      
  reg.lin
  ...
  table.tot.tmp <- data.frame(species = "mouse",
                             ...
                             tissue = unique(full.dataframe.tot$tissue)[i],
                             ...
                             parameters = "-log10(rhythm.p-value) ~ log(mean.RNA.level) + tau")
...
####################################
species <- "baboon"
tissue.list <- c("liver", "lung", "kidney", "muscle", "aorta", "brain_stem", "cerebellum", "heart", "white_adipose")
p.val <- "default.pvalue"
######
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  min.value = min(sub.dataframe.tissue[sub.dataframe.tissue$GeneCycle.pvalue != 0, "GeneCycle.pvalue"])
  sub.dataframe.tissue[sub.dataframe.tissue$GeneCycle.pvalue == 0, "GeneCycle.pvalue"] <- min.value/10
  # idem for RNA expr level, but divided by 1000
  min.value = min(sub.dataframe.tissue[sub.dataframe.tissue$mean.RNA.level != 0, "mean.RNA.level"])
  sub.dataframe.tissue[sub.dataframe.tissue$mean.RNA.level == 0, "mean.RNA.level"] <- min.value/1000
  ...
  table.tot.tmp <- NULL
  table.tot.tmp <- data.frame(species = "baboon",
                              omics = "transcripts", 
                              technique = "RNAseq",
                              tissue = unique(full.dataframe.tot$tissue)[i],
                              `nb tissues` = length(tissue.list),
...
# Rhythmic transcripts p<0.01  /  Not Rhythmic transcripts p>0.5
RNA_rhythm.pvalue_threshold.inf = 0.05
RNA_rhythm.pvalue_threshold.sup = 0.3
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  sub.dataframe.tissue[sub.dataframe.tissue$GeneCycle.pvalue == 0, "GeneCycle.pvalue"] <- min.value/10
  # idem for RNA expr level, but divided by 1000
  min.value = min(sub.dataframe.tissue[sub.dataframe.tissue$mean.RNA.level != 0, "mean.RNA.level"], na.rm = TRUE)
  sub.dataframe.tissue[sub.dataframe.tissue$mean.RNA.level == 0, "mean.RNA.level"] <- min.value/1000
  ...
  table.tot.tmp <- NULL
  table.tot.tmp <- data.frame(species = "drosophila",
                              omics = "transcripts", 
                              technique = "RNAseq",
                              tissue = unique(full.dataframe.tot$tissue)[i],
                              `nb tissues` = length(tissue.list),
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  table.tot.tmp <- NULL
  table.tot.tmp <- data.frame(species = "anopheles",
                              omics = "transcripts", 
                              technique = "microarray",
                              tissue = unique(full.dataframe.tot$tissue)[i],
                              `nb tissues` = length(tissue.list),
...
full.dataframe.tot <- merge(full.dataframe, tot.data[, c("ID", "tau.mean", "tau.max", "nb.rhythmic.tissues")], 
                            by.x="Gene.Name", by.y="ID")
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  table.tot.tmp <- data.frame(species = "mouse",
                              ...
                              technique = "SILAC",
                              tissue = unique(full.dataframe.tot$tissue)[i],
                              `nb tissues` = length(unique(full.dataframe.tot$tissue)),
...
write.csv(table.tot.1, "~/Documents/cost_theory_workingspace/TableS3a.csv", quote = F, row.names = FALSE)
write.csv(table.tot.2, "~/Documents/cost_theory_workingspace/TableS3b.csv", quote = F, row.names = FALSE)
library(xtable)
print(xtable(table.tot.1, type = "latex"), file = "~/Documents/cost_theory_workingspace/TableS3a.tex", include.rownames=FALSE)
print(xtable(table.tot.2, type = "latex"), file = "~/Documents/cost_theory_workingspace/TableS3b.tex", include.rownames=FALSE)
