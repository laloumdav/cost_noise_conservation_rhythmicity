...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  #plot(sub.dataframe.tissue$GeneCycle.pvalue,
  #     sub.dataframe.tissue$tau.mean)
  
  #pearson.cor.test <- cor.test(tot.data$tau.mean, 
  #                             tot.data$nb.rhythmic.tissues, method = "pearson")
  spearman.cor.test <- cor.test(sub.dataframe.tissue$tau.mean, 
                                sub.dataframe.tissue$GeneCycle.pvalue, method = "spearman")
  ...
  table.tot <- rbind(table.tot, data.frame(species = "mouse",
                                           ...
                                           parameters = "tau VS rhythm p-value",
                                           ...
                                           `Spearman_rho` = round(spearman.cor.test$estimate, 2),
                                           `Spearman_S` = formatC(spearman.cor.test$statistic, format = "e", digits = 1),
                                           `Spearman_Signif` = signiFunction(spearman.cor.test$p.value)
                                           #,
                                           #`Pearson_cor` = round(pearson.cor.test$estimate, 2),
                                           #`Pearson_t` = round(pearson.cor.test$statistic, 2),
                                           #`Pearson_df` = round(pearson.cor.test$parameter, 2),
                                           #`Pearson_lower` = round(pearson.cor.test$conf.int[1], 3),
                                           #`Pearson_upper` = round(pearson.cor.test$conf.int[2], 3), 
                                           #`Pearson_Signif` = signiFunction(pearson.cor.test$p.value)
                                           ))
}
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  table.tot <- rbind(table.tot, data.frame(species = "baboon",
                                           omics = "transcript", 
                                           technique = "RNAseq",
                                           tissue = unique(full.dataframe.tot$tissue)[i],
                                           `nb tissues` = length(tissue.list),
...
for (i in 1:length(unique(full.dataframe.tot$tissue))) {
  ...
  #                             tot.data$nb.rhythmic.tissues, method = "pearson")
  spearman.cor.test <- cor.test(sub.dataframe.tissue$tau.mean, 
                                sub.dataframe.tissue$protein_rhythm.pvalue, method = "spearman")
...
write.csv(table.tot, "~/Documents/cost_theory_workingspace/Table3.csv", quote = F, row.names = FALSE)
library(xtable)
print(xtable(table.tot, type = "latex"), file = "~/Documents/cost_theory_workingspace/Table3.tex", include.rownames=FALSE)
